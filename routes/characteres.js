...
router.get("/all", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${process.env.MARVEL_URI}/characters?apiKey=${process.env.MARVEL_API_KEY}&skip=${skip}&limit=${limit}&name=${name}`
    );
...
router.post("/favoris", async (req, res) => {
  try {
    ...
    for (let i = 0; i < favorisId.length; i++) {
      try {
        const response = await axios.get(
          `${process.env.MARVEL_URI}/comics/${favorisId[i]}?apiKey=${process.env.MARVEL_API_KEY}`
        );
